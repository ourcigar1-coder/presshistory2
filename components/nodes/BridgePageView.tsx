...
export function BridgePageView({ data }: { data: BridgePageData }) {
  return (
    <>
      ...
      {data.commonProblem ? (
        <>
          <section aria-labelledby="common-heading" className="mt-12 rounded-2xl border-l-4 border-water bg-water/5 p-5"><h2 id="common-heading" className="text-lg font-bold">{data.commonProblem.heading}</h2><div className="mt-2 text-sm leading-relaxed md:text-base"><RichText value={data.commonProblem.body} /></div></section>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <figure className="overflow-hidden rounded-2xl border border-stone-line">
              <Image src="/images/silicon-wafer.jpg" alt="실리콘 웨이퍼 표면에 반복되는 미세 회로 패턴" width={1200} height={800} className="h-auto w-full object-cover" />
      ...
      <SideTrackChips relations={data.relatedNodes ?? []} />
